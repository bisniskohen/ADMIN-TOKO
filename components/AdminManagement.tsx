...
// Fix: Import newly added Admin and NewAdmin types
...
const AdminManagement: React.FC = () => {
    ...
    const [currentItem, setCurrentItem] = useState<Admin | null>(null);
    ...
    const openModal = (item: Admin | null = null) => {
        setCurrentItem(item);
        setFormData(item ? { adminName: item.adminName } : { adminName: '' });
        setIsModalOpen(true);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (currentItem) {
            await updateDoc(doc(db, "ADMIN", currentItem.id), { ...formData });
        } else {
            await addDoc(collection(db, "ADMIN"), { ...formData, createdAt: serverTimestamp() });
        }
        closeModal();
    };

    const handleDelete = async (id: string) => {
        if (window.confirm("Apakah Anda yakin ingin menghapus admin ini? Toko yang terkait dengan admin ini mungkin perlu diperbarui.")) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h2 className="text-3xl font-bold text-gray-800">Manajemen Admin</h2>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                {['Nama Admin', 'Tanggal Dibuat', 'Aksi'].map(h => (
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {loading ? (
                                ...
                                <tr><td colSpan={3} className="text-center py-4 text-gray-500">Tidak ada data admin.</td></tr>
                            ) : (
                                admins.map((item) => (
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-lg">
                        <form onSubmit={handleSubmit}>
                            <div className="p-6">
                                <h3 className="text-lg font-medium leading-6 text-gray-900">{currentItem ? 'Ubah Data Admin' : 'Tambah Admin Baru'}</h3>
